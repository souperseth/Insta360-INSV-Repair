# Maneuver Analysis Reference

This document describes the analyzer implementation: functionality, data structures, variables, algorithms, plotting internals, CLI flags, and performance notes. For task-oriented commands, see [MANEUVER_ANALYSIS_USAGE.md](MANEUVER_ANALYSIS_USAGE.md).

## Source Layout

| Path | Role |
|------|------|
| `src/insv_tools/accelerometer.py` | Reads Insta360 trailer IMU records and exports accelerometer/gyro time series. |
| `src/insv_tools/maneuver_analysis.py` | Derives motion signals, detects candidate maneuver intervals, estimates damping, writes outputs, and renders plots. |
| `insv_maneuver_analysis.py` | Thin compatibility launcher for the analyzer CLI. |
| `analysis_outputs/` | Default destination for generated summaries, event JSON, feature CSV, and plot images. |

## Key Defaults And Constants

| Constant / CLI default | Value | Meaning |
|------------------------|-------|---------|
| `DEFAULT_INPUT_PATH` | `../sample_insvs_x4/infinite_and_helis.insv` | Default input used when no file is provided. |
| `DEFAULT_SUMMARY_OUTPUT` | `analysis_outputs/infinite_and_helis_summary.txt` | Default human-readable summary path. |
| `DEFAULT_SMOOTH_SECONDS` | `0.50` | Smoothing window for detection signals. |
| `DEFAULT_MIN_DURATION_SECONDS` | `0.75` | Minimum candidate segment duration. |
| `DEFAULT_MERGE_GAP_SECONDS` | `0.50` | Gap below which adjacent candidate segments merge. |
| `DEFAULT_HIGH_G_THRESHOLD` | `2.0` | G-force threshold for load-pulse candidates. |
| `DEFAULT_ROTATION_THRESHOLD` | `2.0` | Gyro-norm threshold for fast-rotation candidates. |
| `DEFAULT_SINGLE_AXIS_DOMINANCE` | `0.58` | Minimum dominant-axis fraction for single-axis spin candidates. |
| `DEFAULT_LOAD_PEAK_SPACING_SECONDS` | `0.35` | Minimum spacing between accepted high-G peaks. |
| `DEFAULT_WINDOW_SECONDS` | `2.0` | ML feature window width. |
| `DEFAULT_STEP_SECONDS` | `0.5` | ML feature window step. |
| `DEFAULT_PLOT_METRICS` | `g_force`, `gyro_norm` | Metrics shown by default in the main timeline. |
| `DEFAULT_PLOT_MAX_POINTS` | `10000` | Default downsample limit per plotted line. |
| `STANDARD_GRAVITY_M_S2` | `9.80665` | Conversion factor for reporting g-force as acceleration magnitude. |

## Functional Overview

The analyzer is a conservative candidate generator, not a final classifier. It
computes derived motion signals from IMU samples, detects threshold-based time
intervals, enriches those intervals with statistics and load-pulse details, and
exports data for review or later supervised labeling.

The launcher GUI reuses the same analysis path for its stats panel, so previewed
values match CLI summaries and plotted windows. It does not report true speed;
the current parser does not extract GPS, velocity, optical-flow, or video-derived
speed data.


This section documents the algorithms, data structures, and internal logic of
`src/insv_tools/maneuver_analysis.py` in detail.
...
```
INSV file
  → insv_tools.accelerometer.read_accelerometer_timeseries()     [raw IMU frames]
...
| `--no-plot`       | Disable the default interactive GUI      |
...
| `--plot-raw`           | on            | Skip normalization                     |
| `--plot-normalized`    | off           | Use robust-normalized metric values    |
| `--plot-max-points`    | 10000         | Downsample limit per line              |
...
- **Matplotlib backend**: Selected early at import time — native `MacOSX` first
  on macOS, then `TkAgg`/`QtAgg`, with `Agg` for file-only output.
- **Interactive plotting**: GUI plots cap rendered points lower than saved
  images and use a fixed layout so Matplotlib toolbar pan/zoom stays responsive.
